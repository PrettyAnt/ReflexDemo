...
import com.part1.anno.MyAnnotation2;
import com.part1.anno.MyAnnotationTest;
...
public class Test0 {
    ...
    public static void main(String[] args) {
        ...
        try {
            clazz = Class.forName("com.part1.bean.Student");
        ...
        try {
            ...
            System.out.println("构造器方法:" + constructor);
            // 使用constructor创造对象
            Object newInstance = constructor.newInstance("wangbei", 24);
            if (newInstance instanceof People) {
                System.out.println("得到的结果-->>" + newInstance);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        for (Field field : fields) {
            System.out.println("变量为:" + field.toString() + " -- 变量名:" + field.getName() + " -- size: " + fields.length);
        }
        System.out.println("\n--------------------指定方法获取-变量------------------------");
        // 指定方式获取
        Field field = null;
        try {
            ...
            System.out.println("变量为:" + field + " -- 变量名:" + field.getName() + " -- size: " + fields.length);
        } catch (Exception e1) {
            e1.printStackTrace();
        }
...
//            System.out.println("反射-->> "+ann);
            if (ann instanceof MyAnnotation) {
                MyAnnotation ann2 = (MyAnnotation) ann;
                System.out.println("注解测试1：" + ann2.name() + "---" + ann2.id() + "---" + ann2.gid());
            }
        }
        // 直接获取类的指定的注解
        ...
        System.out.println("注解测试2：" + name + "---" + id + "---" + gid.toString());
        ...
        try {
            ...
            System.out.println("注解测试3：" + annotation2.name() + "---" + annotation2.id() + "---" + annotation2.gid());
